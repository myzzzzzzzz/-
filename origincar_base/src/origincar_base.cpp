...
std::string shutdown_usart_port_name = "/dev/ttyACM0";
int shutdown_serial_baud_rate = 115200;
...
void origincar_base::Publish_ImuSensor()
{
    ...
    Imu_Data_Pub.angular_velocity_covariance[0] = 0.20f;
    Imu_Data_Pub.angular_velocity_covariance[4] = 0.20f;
    Imu_Data_Pub.angular_velocity_covariance[8] = 0.05f;
    ...
    Imu_Data_Pub.linear_acceleration_covariance[0] = 0.50f;
    Imu_Data_Pub.linear_acceleration_covariance[4] = 0.50f;
    Imu_Data_Pub.linear_acceleration_covariance[8] = 0.50f;
...
bool origincar_base::Get_Sensor_Data()
{
    ...
    for (int retry = 0; retry < RECEIVE_DATA_SIZE * 2; ++retry) {
      if (Stm32_Serial.read(&byte, 1) != 1) {
        return false;
      }
      ...
      Receive_Data.rx[0] = byte;
      if (Stm32_Serial.read(Receive_Data.rx + 1, RECEIVE_FRAME_SIZE - 1) != RECEIVE_FRAME_SIZE - 1) {
        return false;
      }

      if (Receive_Data.rx[RECEIVE_FRAME_SIZE - 1] == FRAME_TAIL &&
          Receive_Data.rx[RECEIVE_FRAME_SIZE - 2] == Check_Sum(RECEIVE_FRAME_SIZE - 2, READ_DATA_CHECK)) {
        frame_ok = true;
        break;
      }

      if (retry == RECEIVE_DATA_SIZE * 2 - 1) {
        return false;
      }
    }
    ...
    Receive_Data.Frame_Tail = Receive_Data.rx[RECEIVE_FRAME_SIZE - 1];
    if (Receive_Data.Frame_Header == FRAME_HEADER) {
      if (Receive_Data.Frame_Tail == FRAME_TAIL) {
        if (Receive_Data.rx[RECEIVE_FRAME_SIZE - 2] == Check_Sum(RECEIVE_FRAME_SIZE - 2, READ_DATA_CHECK)) {
...
: rclcpp::Node ("origincar_base")
{
  ...
  shutdown_usart_port_name = usart_port_name;
  shutdown_serial_baud_rate = serial_baud_rate;

  odom_publisher = create_publisher<nav_msgs::msg::Odometry>("odom", 10);

  imu_publisher = create_publisher<sensor_msgs::msg::Imu>("imu/data", 10);
  ...
  if(Stm32_Serial.isOpen()) {
    RCLCPP_INFO(this->get_logger(),"origincar_base serial port opened: %s", usart_port_name.c_str());
  }
...
void sigintHandler(int sig)
{
    ...
    Stm32_Serial.setPort(shutdown_usart_port_name);
    Stm32_Serial.setBaudrate(shutdown_serial_baud_rate);
    serial::Timeout _time = serial::Timeout::simpleTimeout(20);
    Stm32_Serial.setTimeout(_time);
    Stm32_Serial.open();                                       
    SEND_DATA Send_Data;
